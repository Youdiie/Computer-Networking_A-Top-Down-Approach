...
즉, 클라이언트는 **자신의 상황에 알맞은 비디오 버전을 요청**한다.
...
즉, **최대한 서버를 사용자 근처에 위치**시켜 링크 및 라우터를 거치는 횟수를 줄여 지연시간 및 처리율을 개선하는 것이다.
...
Limelight와 다른 회사들에 의해 적용된 것으로, **좀 더 적은 수의 핵심 지점에 큰 규모의 서버 클러스터를 구축**하여 ISP를 Home으로 가져오는 개념이다.
...
### 넷플릭스 오픈 커넥트
![image](https://github.com/Youdiie/Computer-Networking_A-Top-Down-Approach/assets/87409148/8d8f9668-e51a-4f01-9191-dd62aae87d2f)

넷플릭스 오픈 커넥스 : 자체 CDN

인터넷 서비스 제공업체(ISP)와 협업해 넷플릭스 콘텐츠 제공 방식의 효율성을 높임.

* 제3자 콘텐츠 제공 네트워크가 넷플릭스 콘텐츠를 제공 -> 회원들의 넷플릭스 사용 형태에 대한 지식을 바탕으로 보다 효율적인 콘텐츠 제공이 가능

* 콘텐츠 라이브러리를 구성하는 어마어마한 파일의 수와 용량

  -> ISP의 네트워크에 캐시서버를 설치하고 회원들이 자주 시청하는 콘텐츠를 **새벽 시간대에 미리 저장**해두는 방식

   -> 넷플릭스 회원과 가까운 곳에 저장해 둔 콘텐츠를 스트리밍하기 때문에 넷플릭스로 인해 발생하는 트래픽을 현저히 낮춤 + 먼 거리로 데이터를 전송하는 비용을 절감 + 더 빠른 속도로 고품질의 영상을 제공
...
![image](https://github.com/Youdiie/Computer-Networking_A-Top-Down-Approach/assets/87409148/42b64a0a-fe42-4ca4-af8f-78d072182a0e)

* 검색 : enter-depp
* 비디오 : bring-home

비디오 자체는 bring-home 클러스터 캐시에서 서비스, 
관련된 웹페이지 부분은 가까운 enter-deep 클러스터 캐시로부터, 비디오 관련 광고는 메가 데이터 센터로부터 서비스 됨.
